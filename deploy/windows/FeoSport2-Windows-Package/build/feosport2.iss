...
[Dirs]
; Гарантируем существование logs/ — ярлык "папка с логами" может быть открыт
; до первого запуска FeoSport2 (start-feosport.bat/setup-db.ps1 создают её сами,
; но Inno здесь создаёт пустую заранее, чтобы explorer не падал на ENOENT).
Name: "{app}\logs"
...
Name: "{group}\{#AppName} — папка с логами";          Filename: "explorer.exe"; Parameters: """{app}\logs"""; IconFilename: "{sys}\shell32.dll"; IconIndex: 4
...
begin
  ...
  MsgBox('Файлы FeoSport2 удалены, но база PostgreSQL оставлена нетронутой.' + #13#10 + #13#10 +
         'Для удаления базы выполните вручную через pgAdmin 4 или psql:' + #13#10 +
         '  DROP DATABASE feosport2;' + #13#10 +
         '  DROP ROLE feosport;' + #13#10 + #13#10 +
         'psql найден здесь:' + #13#10 + PsqlPath,
         mbInformation, MB_OK);
end;
...
begin
  ...
  UserChoice := MsgBox(
    ...
    'Да — удалить файлы и показать инструкцию по ручному удалению БД.' + #13#10 +
  ...
  { БД — Inno Setup не имеет safe password prompt на uninstall; показываем инструкцию. }
