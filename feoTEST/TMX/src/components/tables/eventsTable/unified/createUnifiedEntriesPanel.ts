...
export function createUnifiedEntriesPanel({
  ...
  const renderEventControlBar = () => {
    ...
    const ALL_ENTRIES = t('events.allEntries', 'All entries');
    ...
    const items = [
      {
        ...
        placeholder: t('events.searchEntries', 'Search entries'),
        id: 'unifiedSearch',
        location: LEFT,
        search: true,
      },
      ...
      { label: drawName || ALL_ENTRIES, options: entriesOptions, location: LEFT },
      {
        ...
        intent: 'is-warning',
        id: 'editAvoidances',
        label: t('events.avoidances', 'Avoidances'),
        location: RIGHT,
      },
      {
        onClick: () => addFlights({ eventId, callback: () => navigateToEvent({ eventId }) }),
        intent: 'is-info',
        label: t('events.addFlights', 'Add flights'),
        location: RIGHT,
        hide: !!drawId,
      },
      {
        ...
        intent: 'is-primary',
        label: t('events.addDraw', 'Add draw'),
        location: RIGHT,
      },
    ];
